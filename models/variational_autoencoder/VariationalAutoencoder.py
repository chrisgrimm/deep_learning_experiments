...
def log_normal_pdf(x, mu, diag_sigmas):
    ...
    return (-D / 2)*tf.log(2*np.pi) - 0.5*tf.reduce_sum(2*tf.log(diag_sigmas + 10**-10), reduction_indices=1) + exp_part
...
def VAE(input, hiddenSize, codeSize, rvType, prefix=''):
    ...
    if rvType == 'gaussian':
        ...
        q_sigma = tf.abs(tf.add(tf.matmul(h1, W2_sigma), b2_sigma)) + 0.5
