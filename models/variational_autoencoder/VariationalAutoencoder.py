...
def log_normal_pdf(x, mu, diag_sigmas):
    ...
    return (-D**2 / 2)*tf.log(2*np.pi) - 0.5*tf.reduce_sum(2*tf.log(diag_sigmas), reduction_indices=1) + exp_part
...
def VAE(input, hiddenSize, codeSize, rvType):
    ...
    if rvType == 'gaussian':
        ...
        q_sigma = tf.abs(tf.add(tf.matmul(h1, W2_sigma), b2_sigma) + 1)
        return [q_mu, q_sigma]
...
def VAE_realize(params, random_placeholder, rvType):
    if rvType == 'gaussian':
        return params[0] + params[1] * random_placeholder
    elif rvType == 'bernoulli':
